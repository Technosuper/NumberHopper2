...
function bouncebad () {
    for (let value of tiles.getTilesByType(assets.tile`myTile6`)) {
        ...
        Bat_stringer = darts.create(assets.image`bat stinger`, SpriteKind.Projectile)
        Bat_stringer.angleRate = spriteutils.angleFrom(Eviler_musher1, Birdy_1_duck)
        Bat_stringer.angleRate = spriteutils.angleFrom(Eviler_musher1, Birdy_2_robin)
        Bat_stringer.throwDart()
    }
}
sprites.onOverlap(SpriteKind.Player, SpriteKind.Projectile, function (sprite, otherSprite) {
    Bat_stringer.stopDart()
